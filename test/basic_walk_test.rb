...
class BasicWalkTest < Minitest::Test
  ...
  def test_walk_of_single_element
    filter = PandocObjectFilters::Filter.new { |type, value| @types << type; @values << value; nil }
  ...
  def test_walk_replacing_certain_elements
    filter = PandocObjectFilters::Filter.new { |type, value| @types << type; @values << value; space_ast if type == 'Str' }
    result = filter.walk([para_ast(hello_str_ast, space_ast, world_str_ast)])
    assert_equal(['Para', 'Str', 'Space', 'Str'], @types)
    assert_equal([[hello_str_ast, space_ast, world_str_ast], 'hello', [], 'world'], @values)
  ...
  def test_walk_replacing_certain_elements_with_nested_elements
    filter = PandocObjectFilters::Filter.new { |type, value| @types << type; @values << value; plain_ast(soft_break_ast) if type == 'Str' }
    result = filter.walk([para_ast(hello_str_ast, space_ast, world_str_ast)])
  ...
  def test_walk_of_hash_of_elements_replacing_some_elements
    filter = PandocObjectFilters::Filter.new { |type, value| @types << type; @values << value; line_break_ast if type == 'Null' }
    result = filter.walk('x' => [space_ast], 'y' => [soft_break_ast], 'z' => [null_ast])
    assert_equal(['Space', 'SoftBreak', 'Null'], @types)
    assert_equal([[], [], []], @values)
  ...
  def test_walk_and_remove_element
    filter = PandocObjectFilters::Filter.new { |type, value| @types << type; @values << value; [] if type == 'Str' }
    result = filter.walk([para_ast(hello_str_ast, space_ast, world_str_ast)])
    assert_equal(['Para', 'Str', 'Space', 'Str'], @types)
    assert_equal([[hello_str_ast, space_ast, world_str_ast], 'hello', [], 'world'], @values)
  ...
  def test_walk_and_add_elements
    filter = PandocObjectFilters::Filter.new { |type, value| @types << type; @values << value; [line_break_ast, ast(type, value)] if type == 'Str' }
    result = filter.walk([para_ast(hello_str_ast, space_ast, world_str_ast)])
    assert_equal(['Para', 'Str', 'Space', 'Str'], @types)
    assert_equal([[hello_str_ast, space_ast, world_str_ast], 'hello', [], 'world'], @values)
